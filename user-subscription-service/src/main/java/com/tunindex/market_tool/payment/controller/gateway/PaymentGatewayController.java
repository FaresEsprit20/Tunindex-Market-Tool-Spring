...
import com.tunindex.market_tool.payment.dto.promo.ApplyPromoCodeRequestDto;
import com.tunindex.market_tool.payment.dto.promo.ApplyPromoCodeResponseDto;
...
public class PaymentGatewayController implements PaymentGatewayApi {
    ...
    @Override
    public ResponseEntity<ApplyPromoCodeResponseDto> applyPromoCode(ApplyPromoCodeRequestDto request) {
        log.info("POST /api/payments/apply-promo - Code: {}, Amount: {}", request.getCode(), request.getAmount());
        ApplyPromoCodeResponseDto response = promoCodeService.applyPromoCode(request);
        return ResponseEntity.ok(response);
    }
